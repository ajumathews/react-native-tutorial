...
const DATABASE_ID = "68992cfb00130953b8cd";
const COLLECTION_ID = "68992d03000f70d9d59e";
...
export function BooksProvider({ children }) {
  ...
  async function createBook(data) {
    try {
      await databases.createDocument(DATABASE_ID, COLLECTION_ID, ID.unique(), { ...data, userId: user.$id }, [Permission.read(Role.user(user.$id)), Permission.update(Role.user(user.$id)), Permission.delete(Role.user(user.$id))]);
    } catch (error) {
      console.log(error.message);
    }
  }
